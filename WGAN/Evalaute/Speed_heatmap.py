...
for z in range(4):
    for n in range(len(real)):
        ...
        for x in range(49):
            ...
            dist_sample.append([p1x[x], p1y[x], np.floor(velocity)])
            #Sample
            velocity = np.rint(dist2)
            distGen_sample.append([b1x[x], b1y[x], np.floor(velocity)])
            #VAE
            velocity = np.rint(dist4)
            distVae_sample.append([v1x[x], v1y[x], np.floor(velocity)])
            #WGAN
            velocity = np.rint(dist5)
            distWgan_sample.append([w1x[x], w1y[x], np.floor(velocity)])
...
fig.suptitle('Defensive Mean Velocity')
