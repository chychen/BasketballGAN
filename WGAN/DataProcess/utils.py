...
class DataFactory(object):
    ...
    def __normalize_pos(self):
        ...
        for i, axis_ in enumerate(axis_list):
            if axis_ == 'z':  # z
                ...
                self.__real_data[:, :, 0, i] /= 5.0
                self.__real_data[:, :, 0, i] += 0.5
                norm_dict[axis_] = {}
                ...
                self.__real_data[:, :, :, i] /= 5.0
                self.__real_data[:, :, :, i] += 0.5

                self.BASKET_LEFT[i] = (
                                          self.BASKET_LEFT[i] - mean_) / stddev_
                self.BASKET_LEFT[i] /=5.0
                self.BASKET_LEFT[i] +=0.5

                self.BASKET_RIGHT[i] = (
                                           self.BASKET_RIGHT[i] - mean_) / stddev_
                self.BASKET_RIGHT[i] /= 5.0
                self.BASKET_RIGHT[i] +=0.5
                norm_dict[axis_] = {}
                norm_dict[axis_]['mean'] = mean_
                norm_dict[axis_]['stddev'] = stddev_
    ...
    def normalize(self, input_):
        ...
        """
        # x
        print("x mean: ",self.__norm_dict['x']['mean'])
        print("x stddev: ",self.__norm_dict['x']['stddev'])
        input_[:, :, [0, 2, 4, 6, 8, 10]] = ( input_[:, :,[0, 2, 4, 6, 8, 10]] -
                                                                    self.__norm_dict['x']['mean']) / \
                                                                self.__norm_dict['x']['stddev']
        #input_[:, :, [0, 2, 4, 6, 8, 10]] /= 5.0
        #input_[:, :, [0, 2, 4, 6, 8, 10]] += 0.5
        # y
        print("y mean: ",self.__norm_dict['y']['mean'])
        print("y_stddev:",self.__norm_dict['y']['stddev'])
        input_[:, :, [1, 3, 5, 7, 9, 11]] = (input_[:, :, [1, 3, 5, 7, 9, 11]] -
                                                                     self.__norm_dict['y']['mean']) / \
                                                                 self.__norm_dict['y']['stddev']
        #input_[:, :, [1, 3, 5, 7, 9, 11]] /= 5.0
        #input_[:, :, [1, 3, 5, 7, 9, 11]] += 0.5

        return input_

    def normalize_back(self,input_):
        input_[:, :, [0, 2, 4, 6, 8, 10]] -= 0.5
        input_[:, :, [0, 2, 4, 6, 8, 10]] *= 5.0
        input_[:, :, [0, 2, 4, 6, 8, 10]] = (input_[:, :, [0, 2, 4, 6, 8, 10]]  *
                                             self.__norm_dict['x']['stddev']) \
