...
def conv1d_(input_,
    ...
    with tf.variable_scope(name):
        w = tf.get_variable(
            "w", [1, filter_width,
                  input_.get_shape()[-1], output_channels],
            initializer=tf.random_normal_initializer(stddev=0.02),
            dtype=tf.float32)
        bias = tf.get_variable(
            "b", [output_channels], initializer=tf.constant_initializer(0.0))
        if casual:
            padding = [[0, 0], [(filter_width) - 1 * dilation, 0], [0, 0]]
            padded = tf.pad(input_, padding)
            input_expand = tf.expand_dims(padded, dim=1)
            out = tf.nn.atrous_conv2d(
                input_expand, w, rate=dilation, padding='VALID') + bias
        else:
            input_expand = tf.expand_dims(input_, dim=1)
            out = tf.nn.atrous_conv2d(
                input_expand, w, rate=dilation, padding='SAME') + bias

        return tf.squeeze(out, [1])


def dil_residual_blk(input_,
                     dilation,
                     layer_no,
                     channel_,
                     filter_w,
    ...
    with tf.variable_scope(name_) as scope:
        ...
        dil_conv = conv1d_(
            nonlinear2, channel_, dilation, filter_w, name='dilated_conv')
...
def compute_mmd(x, y, sigma_sqr=1.0):
    ...
    return tf.reduce_mean(x_kernel) + tf.reduce_mean(
        y_kernel) - 2 * tf.reduce_mean(xy_kernel)
...
def get_feature(real_, mean_):
    ...
    data[:, :, [0, 2, 4, 6, 8, 10]] = (
        data[:, :, [0, 2, 4, 6, 8, 10]] * mean_[1]) + mean_[0]
    data[:, :, [1, 3, 5, 7, 9, 11]] = (
        data[:, :, [1, 3, 5, 7, 9, 11]] * mean_[3]) + mean_[2]
    ...
    for x in range(len(data)):
        for i in range(100):
            ...
            basketd = distance(ballx, bally, BASKET_RIGHT[i, 0],
                               BASKET_RIGHT[i, 1])
